...
import { LocalizedText, LocalizedTextInput } from "@/lib/i18n";
...
function WorkerKycView({ workerId, onBack, onSkip, mandatory = false }: { workerId?: string; onBack: () => void; onSkip?: () => void; mandatory?: boolean }) {
  ...
  const pick = async (kind: "id" | "selfie") => { try { const result = await ImagePicker.launchImageLibraryAsync({ mediaTypes: ImagePicker.MediaTypeOptions.Images, quality: 0.8 }); const asset = result.canceled ? undefined : result.assets[0]; if (asset?.uri) { setSubmitDone(false); if (kind === "id") { setIdUri(asset.uri); setIdMimeType(asset.mimeType); } else { setSelfieUri(asset.uri); setSelfieMimeType(asset.mimeType); } } } catch (error) { Alert.alert("Upload unavailable", error instanceof Error ? error.message : "Please choose the file again."); } };
  const submit = async () => { const normalizedId = idNumber.replace(/[^a-zA-Z0-9]/g, ""); if (status === "pending") { Alert.alert("KYC under review", "Your documents are already submitted. The worker dashboard will open only after an administrator approves your verification."); return; } if (!workerId || !idUri || !selfieUri || normalizedId.length < 4) { Alert.alert("Complete KYC details", "Upload your ID document and selfie, then enter your complete ID number."); return; } setBusy(true); try { const [idDocumentUrl, selfieUrl] = await Promise.all([uploadKycDocument(idUri, workerId, "id", idMimeType), uploadKycDocument(selfieUri, workerId, "selfie", selfieMimeType)]); await saveWorkerKyc({ workerId, idType, idNumberLast4: normalizedId.slice(-4), idDocumentUrl, selfieUrl }); setIdNumber(""); setSubmitDone(true); Alert.alert("KYC submitted", "Your documents are securely submitted for admin review."); } catch (error) { Alert.alert("KYC submission failed", error instanceof Error ? error.message : "Please try again."); } finally { setBusy(false); } };
