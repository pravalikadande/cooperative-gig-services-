...
import { Alert, BackHandler, Image, Pressable, ScrollView, StyleSheet, Switch, View, Platform } from "react-native";
...
import { GlobalLanguagePicker, LocalizedText, LocalizedTextInput, translateText } from "@/lib/i18n";
...
export function WorkerOperations({ user, name: legacyName, onSaveProfile, onSignOut }: { user?: AppUser; name?: string; onSaveProfile: (input: { name?: string; phone?: string; address?: string; workerProfile?: Record<string, unknown> }) => Promise<void>; onSignOut: () => Promise<void> }) {
  ...
  const updateOnlineAvailability = (value: boolean) => {
    ...
    onSaveProfile({ workerProfile: { isOnline: value } }).catch(() => {
      setIsOnline(!value);
      Alert.alert(translateText("Availability not updated"), translateText("We could not update your public worker availability. Please try again."));
    });
  };
  const useCurrentServiceArea = async () => {
    const result = await requestCurrentServiceLocation();
    if (!result.ok) {
      Alert.alert(translateText("Area not updated"), translateText(result.message, result.message));
      return;
    }
    ...
    try {
      ...
      Alert.alert(translateText("Service area updated"), `${result.label} ${translateText("is now shown on your public worker profile.")}`);
    } catch (error) {
      Alert.alert(translateText("Area not saved"), error instanceof Error ? translateText(error.message, error.message) : translateText("Please try again."));
    }
  };
  useEffect(() => {
  ...
  const accept = async (request: Job) => {
    ...
    try {
      ...
      Alert.alert(translateText("Request accepted"), `${request.customer} ${translateText("has been notified. Confirm the schedule before starting the job.")}`);
    } catch (error) {
      Alert.alert(translateText("Request not updated"), error instanceof Error ? translateText(error.message, error.message) : translateText("Please try again."));
    }
  };
  const reject = async (request: Job) => {
    try { await updateBookingStatus(request.id, "rejected"); Alert.alert(translateText("Request declined"), translateText("The customer will receive a respectful update.")); } catch (error) { Alert.alert(translateText("Request not updated"), error instanceof Error ? translateText(error.message, error.message) : translateText("Please try again.")); }
  };
  const updateJob = async (job: Job, next: BookingStatus) => {
    if (!canTransitionBooking(job.status, next)) return;
    try { await updateBookingStatus(job.id, next); } catch (error) { Alert.alert(translateText("Job not updated"), error instanceof Error ? translateText(error.message, error.message) : translateText("Please try again.")); return; }
...
function WorkerProfile({ workerId, profile, jobs, earnings, onProfileChange, onSave, onUseCurrentArea, onSignOut, restricted = false }: { workerId?: string; profile: WorkerProfileDraft; jobs: Job[]; earnings: number; onProfileChange: (profile: WorkerProfileDraft) => void; onSave: () => Promise<void>; onUseCurrentArea: () => Promise<void>; onSignOut: () => Promise<void>; restricted?: boolean }) {
  ...
  const save = () => {
    onSave().then(() => {
      setSection("menu");
      Alert.alert(translateText("Profile saved"), translateText("Your worker profile has been saved to your account."));
    }).catch((error) => Alert.alert(translateText("Profile not saved"), error instanceof Error ? translateText(error.message, error.message) : translateText("Please try again.")));
  };
  ...
  return <><Intro title="Worker profile" copy="Keep your cooperative profile accurate and trustworthy." />{!profile.phone.trim() && <Pressable onPress={() => setSection("details")} style={({ pressed }) => [styles.missingPhoneCard, pressed && styles.pressed]}><View style={styles.missingPhoneIcon}><MaterialIcons name="phone" size={20} color="#C2410C" /></View><View style={styles.flex}><Text style={styles.missingPhoneTitle} translationKey="phoneNumberMissingTitle">Phone number missing</Text><Text style={styles.missingPhoneCopy} translationKey="phoneNumberMissingCopy">Add your phone number so the cooperative can contact you about bookings and service updates.</Text></View><Text style={styles.addPhoneText} translationKey="addPhoneNumber">Add phone number</Text></Pressable>}{restricted && <View style={styles.lockedCard}><MaterialIcons name="lock" size={20} color="#C2410C" /><View style={styles.flex}><Text style={styles.lockedTitle}>Complete KYC to unlock your worker tools</Text><Text style={styles.lockedCopy}>Your dashboard, job actions, availability, and profile editing stay disabled until your identity is approved.</Text></View></View>}<View style={styles.workerIdentity}><View style={styles.profileIdentityMain}><View style={styles.profileAvatar}><Text style={styles.profileAvatarText}>{initials(profile.name)}</Text></View><View style={styles.profileIdentityDetails}><View style={styles.identityLine}><Text style={styles.identityName}>{profile.name}</Text><Text style={styles.achievementBadge}>{achievementBadge(jobs.filter((job) => job.status === "completed").length)}</Text></View><Text style={styles.identityCopy}>{profile.services || "Add your services"}</Text><View style={styles.profileInfoLine}><MaterialIcons name="location-on" size={14} color="#0F766E" /><Text style={styles.profileInfoText}>{profile.serviceArea}</Text></View><View style={styles.profileInfoLine}><MaterialIcons name="schedule" size={14} color="#0F766E" /><Text style={styles.profileInfoText}>{profile.availability}</Text></View></View></View><View style={styles.profileRatingBox}><Text style={styles.profileRatingValue}>4.9</Text><Text style={styles.profileRatingStars}>★★★★★</Text><Text style={styles.profileRatingCount}>128 reviews</Text></View></View><WorkerProfileAnalytics jobs={jobs} earnings={earnings} /><SettingsRow icon="person-outline" label="Personal details" onPress={() => setSection("details")} disabled={restricted} /><SettingsRow icon="build" label="Skills & services" onPress={() => setSection("services")} disabled={restricted} /><SettingsRow icon="location-on" label="Service area" onPress={() => setSection("area")} disabled={restricted} /><SettingsRow icon="schedule" label="Availability" onPress={() => setSection("availability")} disabled={restricted} /><SettingsRow icon="visibility" label="Public profile preview" onPress={() => setSection("preview")} disabled={restricted} /><AppearanceSetting /><View style={styles.profileLanguageSection}><View style={styles.profileLanguageCopy}><Text style={styles.settingsLabel} translationKey="language">Language</Text><Text style={styles.settingsHint} translationKey="selectLanguage">Select language</Text></View><GlobalLanguagePicker floating={false} /></View><Pressable onPress={onSignOut} style={({ pressed }) => [styles.signOutRow, pressed && styles.pressed]}><MaterialIcons name="logout" size={20} color="#C2410C" /><Text style={styles.signOutText}>Sign out</Text></Pressable></>;
}
...
function WorkerKycView({ workerId, onBack, onSkip, mandatory = false }: { workerId?: string; onBack: () => void; onSkip?: () => void; mandatory?: boolean }) {
  ...
  const pick = async (kind: "id" | "selfie") => { try { const result = await ImagePicker.launchImageLibraryAsync({ mediaTypes: ImagePicker.MediaTypeOptions.Images, quality: 0.8 }); const asset = result.canceled ? undefined : result.assets[0]; if (asset?.uri) { setSubmitDone(false); if (kind === "id") { setIdUri(asset.uri); setIdMimeType(asset.mimeType); } else { setSelfieUri(asset.uri); setSelfieMimeType(asset.mimeType); } } } catch (error) { Alert.alert(translateText("Upload unavailable"), error instanceof Error ? translateText(error.message, error.message) : translateText("Please choose the file again.")); } };
  const submit = async () => { const normalizedId = idNumber.replace(/[^a-zA-Z0-9]/g, ""); if (status === "pending") { Alert.alert(translateText("KYC under review"), translateText("Your documents are already submitted. The worker dashboard will open only after an administrator approves your verification.")); return; } if (!workerId || !idUri || !selfieUri || normalizedId.length < 4) { Alert.alert(translateText("Complete KYC details"), translateText("Upload your ID document and selfie, then enter your complete ID number.")); return; } setBusy(true); try { const [idDocumentUrl, selfieUrl] = await Promise.all([uploadKycDocument(idUri, workerId, "id", idMimeType), uploadKycDocument(selfieUri, workerId, "selfie", selfieMimeType)]); await saveWorkerKyc({ workerId, idType, idNumberLast4: normalizedId.slice(-4), idDocumentUrl, selfieUrl }); setIdNumber(""); setSubmitDone(true); Alert.alert(translateText("KYC submitted"), translateText("Your documents are securely submitted for admin review.")); } catch (error) { Alert.alert(translateText("KYC submission failed"), error instanceof Error ? translateText(error.message, error.message) : translateText("Please try again.")); } finally { setBusy(false); } };
...
function AdminSettings({ name }: { name: string }) { return <><Intro title="Admin personal details" copy="Manage administrator profile and account settings." /><View style={styles.bookingAdminCard}><Text style={styles.adminBookingService}>{name}</Text><Text style={styles.adminBookingPeople}>Administrator account</Text><Text style={styles.adminBookingPeople}>Role-protected access enabled</Text></View><SettingsRow icon="security" label="Security and authorization" onPress={() => Alert.alert("Security", "Admin access is restricted to provisioned administrator accounts.")} /><SettingsRow icon="notifications" label="Notification preferences" onPress={() => Alert.alert("Notifications", "Admin alerts are enabled for new workers, reports, and payment issues.")} /><AppearanceSetting /></>; }
...
async function openPrivateCall(phone?: string) { const digits = phone?.replace(/\D/g, "") ?? ""; if (digits.length < 10) { Alert.alert("Phone number unavailable", "This customer has not saved a valid contact number yet. Ask them to add it in their customer profile."); return; } const dialablePhone = phone?.trim().startsWith("+") ? `+${digits}` : digits.length === 10 ? `+91${digits}` : `+${digits}`; try { await Linking.openURL(`tel:${dialablePhone}`); } catch (error) { Alert.alert("Calling unavailable", error instanceof Error ? error.message : "A phone app is not available on this device. Try the call from a physical phone."); } }
async function openKycDocument(url: string) { try { if (!(await Linking.canOpenURL(url))) throw new Error("This document link is not available on the device."); await Linking.openURL(url); } catch (error) { Alert.alert("Document unavailable", error instanceof Error ? error.message : "Please try opening the document again."); } }
