...
export function AdminOperations({ name, onSignOut }: { name: string; onSignOut: () => Promise<void> }) { const colors = useColors(); const { t } = useI18n(); const [view, setView] = useState<AdminView>("overview"); const [data, setData] = useState<AdminData>({ users: [], workers: [], bookings: [] }); const [kycItems, setKycItems] = useState<import("@/lib/gig/models").WorkerKyc[]>([]); useEffect(() => { try { return subscribeToAdminData(setData); } catch { return undefined; } }, []); useEffect(() => { try { return subscribeToAllWorkerKyc(setKycItems); } catch { return undefined; } }, []); useEffect(() => { if (Platform.OS !== "android") return undefined; const subscription = BackHandler.addEventListener("hardwareBackPress", () => { if (view !== "overview") { setView("overview"); return true; } return false; }); return () => subscription.remove(); }, [view]); return <View style={[styles.page, { backgroundColor: colors.background }]}><View style={[styles.header, { backgroundColor: colors.surface, borderBottomColor: colors.border }]} ><View><Text style={styles.headerOverline}>{t("ADMIN PORTAL", "ADMIN PORTAL")}</Text><Text style={styles.headerTitle}>{name.split(" ")[0]} — {t("Overview", "Overview")}</Text></View><Pressable onPress={onSignOut} style={({ pressed }) => [styles.headerLogout, pressed && styles.pressed]}><MaterialIcons name="logout" size={19} color="#0F766E" /></Pressable></View><AdminBottomNav active={view} onPress={setView} /><ScrollView contentContainerStyle={[styles.scroll, { backgroundColor: colors.background }]}><ProfileLanguagePicker />{view === "overview" && <AdminOverview data={data} kycItems={kycItems} onPressWorkers={() => setView("workers")} onPressKyc={() => setView("kyc")} onPressReports={() => setView("reports")} />}{view === "workers" && <AdminWorkers workers={data.workers} onOpenKyc={() => setView("kyc")} onBack={() => setView("overview")} />}{view === "kyc" && <AdminKycReview items={kycItems} reviewer={name} />}{view === "users" && <AdminUsers users={data.users} />}{view === "bookings" && <AdminBookings bookings={data.bookings} />}{view === "financials" && <AdminFinancials bookings={data.bookings} />}{view === "locations" && <AdminLocations workers={data.workers} bookings={data.bookings} />}{view === "reports" && <AdminReports />}{view === "settings" && <AdminSettings name={name} />}</ScrollView></View>; }
...
function AdminBottomNav({ active, onPress }: { active: AdminView; onPress: (view: AdminView) => void }) { const tabs: { label: string; view: AdminView; icon: keyof typeof MaterialIcons.glyphMap }[] = [{ label: "Overview", view: "overview", icon: "insights" }, { label: "Workers", view: "workers", icon: "engineering" }, { label: "Users", view: "users", icon: "groups" }, { label: "Bookings", view: "bookings", icon: "event-note" }, { label: "Finance", view: "financials", icon: "payments" }, { label: "Locations", view: "locations", icon: "map" }, { label: "Reports", view: "reports", icon: "flag" }, { label: "Settings", view: "settings", icon: "settings" }]; return <View style={styles.adminNavBar}><ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.adminNavContent}>{tabs.map((tab) => { const selected = active === tab.view; return <Pressable key={tab.label} onPress={() => onPress(tab.view)} style={({ pressed }) => [styles.adminNavItem, selected && styles.adminNavItemActive, pressed && styles.pressed]}><MaterialIcons name={tab.icon} size={20} color={selected ? "#0F766E" : "#627D98"} /><Text style={[styles.adminNavLabel, selected && styles.adminNavLabelActive]}>{tab.label}</Text></Pressable>; })}</ScrollView></View>; }
...
const styles = StyleSheet.create({
  ...
  adminNavBar: { backgroundColor: "#FFFFFF", borderBottomColor: "#D9E2EC", borderBottomWidth: 1, borderTopColor: "#D9E2EC", borderTopWidth: 1, minHeight: 70 }, adminNavContent: { gap: 8, paddingHorizontal: 12, paddingVertical: 10 }, adminNavItem: { alignItems: "center", borderRadius: 12, flexDirection: "row", gap: 6, minHeight: 46, paddingHorizontal: 12 }, adminNavItemActive: { backgroundColor: "#E6FFFA" }, adminNavLabel: { color: "#627D98", fontSize: 11, fontWeight: "700" }, adminNavLabelActive: { color: "#0F766E", fontWeight: "800" },
