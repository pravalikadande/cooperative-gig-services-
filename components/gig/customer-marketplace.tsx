...
import { createBooking, sendBookingMessage, subscribeToBookings, subscribeToMessages, subscribeToWorkerDirectory, submitReview } from "@/lib/gig/firebase-repository";
...
function BookingView({ worker, service, description, setDescription, paymentMethod, setPaymentMethod, serviceLocation, onUseCurrentLocation, bookingImageUri, onChooseImage, onBack, onConfirm, isSubmitting }: { worker: WorkerProfile; service: ServiceCategory; description: string; setDescription: (value: string) => void; paymentMethod: PaymentMethod; setPaymentMethod: (value: PaymentMethod) => void; serviceLocation: { latitude: number; longitude: number; label: string }; onUseCurrentLocation: () => void; bookingImageUri: string | null; onChooseImage: () => void; onBack: () => void; onConfirm: () => void; isSubmitting: boolean }) {
...
function BookingsView({ createdBooking, liveBookings, selectedWorker, selectedService, availableWorkers, onWorker, onMessage, onCall, hasReviewed, onReview }: { createdBooking: { id: string; status: BookingStatus; date: string; time: string } | null; liveBookings: Booking[] | null; selectedWorker: WorkerProfile; selectedService: ServiceCategory; availableWorkers: WorkerProfile[]; onWorker: (worker: WorkerProfile) => void; onMessage: (booking: Booking) => void; onCall: (booking: Booking) => void; hasReviewed: boolean; onReview: (booking: Booking) => void }) {
