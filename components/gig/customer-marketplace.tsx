...
import { GlobalLanguagePicker, LocalizedText, LocalizedTextInput, translateText, useI18n } from "@/lib/i18n";
...
export function CustomerMarketplace({ user, onSaveProfile, onSignOut }: { user: AppUser; onSaveProfile: (input: { name?: string; phone?: string; address?: string }) => Promise<void>; onSignOut: () => Promise<void> }) {
  ...
  useEffect(() => {
    void AsyncStorage.multiGet(["cgs.reviewedBookings", "cgs.paidBookings"]).then(([reviews, payments]) => {
      try { setReviewedBookingIds(JSON.parse(reviews[1] || "[]")); } catch { setReviewedBookingIds([]); }
      try { setPaidBookingIds(JSON.parse(payments[1] || "[]")); } catch { setPaidBookingIds([]); }
    });
  }, []);
  const markReviewed = (bookingId: string) => { setReviewedBookingIds((current) => { const next = current.includes(bookingId) ? current : [...current, bookingId]; void AsyncStorage.setItem("cgs.reviewedBookings", JSON.stringify(next)); return next; }); };
  const markPaid = (bookingId: string) => { setPaidBookingIds((current) => { const next = current.includes(bookingId) ? current : [...current, bookingId]; void AsyncStorage.setItem("cgs.paidBookings", JSON.stringify(next)); return next; }); };
  ...
  const detectCurrentLocation = async (silent = false) => {
    try {
      ...
      if (!result.ok) {
        if (!silent) Alert.alert(t("Location unavailable"), t(result.message, result.message));
        return;
      }
      ...
      if (!silent) Alert.alert(t("Location unavailable"), t("We could not read your current location. Choose a city manually instead."));
    }
  };
  ...
  const confirmBooking = async () => {
    ...
    try {
      ...
      Alert.alert(t("Booking not sent"), error instanceof Error ? t(error.message, error.message) : t("We could not create this booking. Please try again."));
    } finally {
  ...
  return (
    <View style={[styles.page, { backgroundColor: colors.background }]}>
      <View style={[styles.topbar, { backgroundColor: colors.surface, borderBottomColor: colors.border }]} >
        ...
        <Pressable onPress={() => Alert.alert(t("Notifications"), t("No new service updates yet."))} style={({ pressed }) => [styles.circleButton, pressed && styles.pressed]}><MaterialIcons name="notifications-none" size={22} color="#102A43" /></Pressable>
      </View>
...
function BookingsView({ createdBooking, liveBookings, selectedWorker, selectedService, availableWorkers, onWorker, onMessage, onCall, reviewedBookingIds, paidBookingIds, onReview, onPay }: { createdBooking: { id: string; status: BookingStatus; date: string; time: string } | null; liveBookings: Booking[] | null; selectedWorker: WorkerProfile; selectedService: ServiceCategory; availableWorkers: WorkerProfile[]; onWorker: (worker: WorkerProfile) => void; onMessage: (booking: Booking) => void; onCall: (booking: Booking) => void; reviewedBookingIds: string[]; paidBookingIds: string[]; onReview: (booking: Booking) => void; onPay: (booking: Booking) => Promise<void> }) {
...
function DisabledAction({ label, icon, disabled }: { label: string; icon: keyof typeof MaterialIcons.glyphMap; disabled: boolean }) { return <Pressable disabled={disabled} onPress={() => Alert.alert(translateText(label, label), translateText("A valid active booking is required before contact is available."))} style={styles.disabledAction}><MaterialIcons name={icon} size={20} color="#94A3B8" /><Text style={styles.disabledText}>{label}</Text></Pressable>; }
function SelectRow({ icon, label, value, onPress }: { icon: keyof typeof MaterialIcons.glyphMap; label: string; value: string; onPress?: () => void }) { return <Pressable onPress={onPress ?? (() => Alert.alert(translateText(label, label), translateText("Selection controls will be connected to live availability in the Firebase-enabled release.")))} style={({ pressed }) => [styles.selectRow, pressed && styles.pressed]}><View style={styles.selectIcon}><MaterialIcons name={icon} size={20} color="#0F766E" /></View><View style={styles.flex}><Text style={styles.selectLabel}>{label}</Text><Text style={styles.selectValue}>{value}</Text></View><MaterialIcons name="chevron-right" size={22} color="#829AB1" /></Pressable>; }
...
async function openBookingCall(phone?: string) {
  ...
  if (dialablePhone.replace(/\D/g, "").length < 10) {
    Alert.alert(translateText("Phone number unavailable"), translateText("This worker has not saved a valid contact number yet. Ask them to add it in their worker profile."));
    return;
  }
  try {
    ...
    Alert.alert(translateText("Calling unavailable"), error instanceof Error ? translateText(error.message, error.message) : translateText("A compatible phone application is not available on this device."));
  }
}
...
const styles = StyleSheet.create({
  ...
  missingPhoneCard: { alignItems: "center", backgroundColor: "#FFF7ED", borderColor: "#FDBA74", borderRadius: 14, borderWidth: 1, flexDirection: "row", gap: 10, marginTop: 12, padding: 13 }, missingPhoneIcon: { alignItems: "center", backgroundColor: "#FFEDD5", borderRadius: 10, height: 38, justifyContent: "center", width: 38 }, missingPhoneTitle: { color: "#9A3412", fontSize: 13, fontWeight: "800" }, missingPhoneCopy: { color: "#9A3412", fontSize: 11, lineHeight: 16, marginTop: 3 }, addPhoneText: { color: "#C2410C", fontSize: 11, fontWeight: "800", textAlign: "right" }, profileLanguageSection: { alignItems: "center", backgroundColor: "#FFFFFF", borderColor: "#D9E2EC", borderRadius: 14, borderWidth: 1, flexDirection: "row", gap: 12, marginTop: 10, minHeight: 63, paddingHorizontal: 14 }, profileLanguageCopy: { flex: 1 }, customerProfile: { alignItems: "center", backgroundColor: "#FFFFFF", borderColor: "#D9E2EC", borderRadius: 18, borderWidth: 1, marginTop: 23, padding: 21 }, customerName: { color: "#102A43", fontSize: 19, fontWeight: "800", marginTop: 11 }, customerEmail: { color: "#627D98", fontSize: 13, marginTop: 4 }, settingsRow: { alignItems: "center", backgroundColor: "#FFFFFF", borderColor: "#D9E2EC", borderRadius: 14, borderWidth: 1, flexDirection: "row", gap: 12, marginTop: 10, minHeight: 55, paddingHorizontal: 14 }, settingsText: { color: "#334E68", flex: 1, fontSize: 14, fontWeight: "700" }, logOutRow: { alignItems: "center", flexDirection: "row", gap: 11, minHeight: 48, marginTop: 16, paddingHorizontal: 14 }, logOutText: { color: "#C2410C", fontSize: 14, fontWeight: "800" },
