...
function HomeView({ greeting, name, selectedService, setSelectedService, workers, onExplore, onWorker }: { greeting: string; name: string; selectedService: ServiceCategory; setSelectedService: (service: ServiceCategory) => void; workers: WorkerProfile[]; onExplore: () => void; onWorker: (worker: WorkerProfile) => void }) {
  const { t } = useI18n();
  return <>
    <View style={styles.greeting}><Text style={styles.overline}>{t(greeting, greeting).toUpperCase()}</Text><Text style={styles.title}>{t("Hi", "Hi")}, {name.split(" ")[0]}.</Text><Text style={styles.subtle}>{t("What can the cooperative help with today?", "What can the cooperative help with today?")}</Text></View>
    <Pressable onPress={onExplore} style={({ pressed }) => [styles.searchBar, pressed && styles.pressed]}><MaterialIcons name="search" size={22} color="#627D98" /><Text style={styles.searchPlaceholder}>{t("Search a service or worker", "Search a service or worker")}</Text><MaterialIcons name="tune" size={20} color="#0F766E" /></Pressable>
    <SectionHeader title="Services near you" action="See all" onPress={onExplore} />
    <View style={styles.serviceGrid}>{services.map((service) => <Pressable key={service.id} onPress={() => { setSelectedService(service); onExplore(); }} style={({ pressed }) => [styles.serviceCard, service.id === selectedService.id && styles.serviceCardActive, pressed && styles.pressed]}><View style={styles.serviceIcon}><MaterialIcons name={service.icon} size={25} color="#0F766E" /></View><Text style={styles.serviceName}>{t(service.name, service.name)}</Text><Text style={styles.serviceFrom}>{t("From", "From")} ₹{service.startingPrice}</Text></Pressable>)}</View>
...
const styles = StyleSheet.create({
  ...
  phoneAlertCard: { alignItems: "center", backgroundColor: "#FFFBEB", borderColor: "#FCD34D", borderRadius: 14, borderWidth: 1, flexDirection: "row", gap: 11, marginTop: 16, paddingHorizontal: 14, paddingVertical: 13 }, phoneAlertTitle: { color: "#92400E", fontSize: 14, fontWeight: "800" }, phoneAlertCopy: { color: "#A16207", fontSize: 12, lineHeight: 17, marginTop: 3 }, customerProfile: { alignItems: "center", backgroundColor: "#FFFFFF", borderColor: "#D9E2EC", borderRadius: 18, borderWidth: 1, marginTop: 23, padding: 21 }, customerName: { color: "#102A43", fontSize: 19, fontWeight: "800", marginTop: 11 }, customerEmail: { color: "#627D98", fontSize: 13, marginTop: 4 }, settingsRow: { alignItems: "center", backgroundColor: "#FFFFFF", borderColor: "#D9E2EC", borderRadius: 14, borderWidth: 1, flexDirection: "row", gap: 12, marginTop: 10, minHeight: 55, paddingHorizontal: 14 }, settingsText: { color: "#334E68", flex: 1, fontSize: 14, fontWeight: "700" }, logOutRow: { alignItems: "center", flexDirection: "row", gap: 11, minHeight: 48, marginTop: 16, paddingHorizontal: 14 }, logOutText: { color: "#C2410C", fontSize: 14, fontWeight: "800" },
