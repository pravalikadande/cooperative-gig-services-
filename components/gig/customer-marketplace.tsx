...
import { createBooking, sendBookingMessage, subscribeToBookings, subscribeToMessages, subscribeToWorkerDirectory } from "@/lib/gig/firebase-repository";
import { requestCurrentServiceLocation } from "@/lib/gig/location";
import type { AppUser, Booking, BookingStatus, ChatMessage, ServiceCategory, WorkerProfile } from "@/lib/gig/models";
...
type WorkerFilter = "All" | "Online now" | "Verified" | "Top rated";
type CustomerProfileDraft = { name: string; phone: string; address: string; notificationsEnabled: boolean };
...
export function CustomerMarketplace({ user, onSaveProfile, onSignOut }: { user: AppUser; onSaveProfile: (input: { name?: string; phone?: string; address?: string }) => Promise<void>; onSignOut: () => Promise<void> }) {
  ...
  }), [activeWorkerFilter, directoryWorkers, query]);
  ...
  const saveCustomerProfile = async () => {
    await onSaveProfile({ name: customerProfile.name, phone: customerProfile.phone, address: customerProfile.address });
  };
...
function ExploreView({ query, setQuery, activeFilter, setActiveFilter, workers, onWorker }: { query: string; setQuery: (value: string) => void; activeFilter: WorkerFilter; setActiveFilter: (filter: WorkerFilter) => void; workers: WorkerProfile[]; onWorker: (worker: WorkerProfile) => void }) {
  const filters: WorkerFilter[] = ["All", "Online now", "Verified", "Top rated"];
...
function BookingsView({ createdBooking, liveBookings, selectedWorker, selectedService, availableWorkers, onWorker, onMessage, onCall, hasReviewed, onReview }: { createdBooking: { id: string; status: BookingStatus; date: string; time: string } | null; liveBookings: Booking[] | null; selectedWorker: WorkerProfile; selectedService: ServiceCategory; availableWorkers: WorkerProfile[]; onWorker: (worker: WorkerProfile) => void; onMessage: (booking: Booking) => void; onCall: (booking: Booking) => void; hasReviewed: boolean; onReview: () => void }) {
...
function ProfileView({ profile, onProfileChange, onSave, onSignOut }: { profile: CustomerProfileDraft; onProfileChange: (profile: CustomerProfileDraft) => void; onSave: () => Promise<void>; onSignOut: () => Promise<void> }) {
