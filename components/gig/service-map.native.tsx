...
export function ServiceMap({ latitude, longitude, label }: { latitude: number; longitude: number; label: string }) {
  if (!shouldRenderNativeGoogleMap(process.env.EXPO_PUBLIC_GOOGLE_MAPS_API_KEY)) {
    return <View style={styles.mapFallback}><View style={styles.pin}><MaterialIcons name="location-on" size={24} color="#0F766E" /></View><Text style={styles.label}>{label}</Text><Text translationKey="Service location saved. Interactive map will appear after Google Maps is configured." style={styles.copy}>Service location saved. Interactive map will appear after Google Maps is configured.</Text></View>;
  }
